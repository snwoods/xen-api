...
let queue_operation_int ?traceparent ?(redirector = Redirector.default) dbg id op =
  let task =
    ...
    Debug.with_thread_associated dbg
      (fun () ->
        ...
        | Some transferred_fd ->
            ...
            in
            let task = Some (queue_operation ?traceparent ~redirector:Redirector.receive_memory_queues dbg id op) in
            Option.iter

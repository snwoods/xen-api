...
let options =
  ...
  ; ( "xenopsd-vbd-plug-legacy"
    , Arg.Bool (fun x -> Xenops_server.xenopsd_vbd_plug_legacy := x)
    , (fun () -> string_of_bool !Xenops_server.xenopsd_vbd_plug_legacy)
    , "False if we want to split the plug atomic into attach/activate"
    )
  ; ( "xenopsd-vbd-unplug-legacy"
    , Arg.Bool (fun x -> Xenops_server.xenopsd_vbd_unplug_legacy := x)
    , (fun () -> string_of_bool !Xenops_server.xenopsd_vbd_unplug_legacy)
    , "False if we want to split the unplug atomic into deactivate/detach"
    )
  ]

...
module ObserverAPI (R : RPC) = struct
  ...
  let destroy =
    declare "Observer.destroy" []
      (dbg_p @-> uuid_p @-> returning unit_p err)
  ...
  let init =
    declare "Observer.init" [] (dbg_p @-> returning unit_p err)
...
module Server (Impl : Server_impl) () = struct
  ...
  let _ =
    S.create (fun dbg uuid name_label attributes endpoints enabled ->
        Impl.create () ~dbg ~uuid ~name_label ~attributes ~endpoints
          ~enabled
    ) ;
    ...
    S.set_trace_log_dir (fun dbg dir ->
        Impl.set_trace_log_dir () ~dbg ~dir
    ) ;
    S.set_export_interval (fun dbg interval ->
        Impl.set_export_interval () ~dbg ~interval
    ) ;
    S.set_max_spans (fun dbg spans ->
        Impl.set_max_spans () ~dbg ~spans
    ) ;
    S.set_max_traces (fun dbg traces ->
        Impl.set_max_traces () ~dbg ~traces
    ) ;
    S.set_max_file_size (fun dbg file_size ->
        Impl.set_max_file_size () ~dbg ~file_size
    ) ;
    S.set_host_id (fun dbg host_id ->
        Impl.set_host_id () ~dbg ~host_id
    ) ;

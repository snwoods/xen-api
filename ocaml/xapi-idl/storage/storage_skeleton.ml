...
module Observer = struct
  let create ctx ~dbg ~uuid ~name_label ~attributes ~endpoints ~enabled =
    D.info "skeleton Observer.create" ;
    ()

  let destroy ctx ~dbg ~uuid = D.info "skeleton Observer.destroy" ; ()

  let set_enabled ctx ~dbg ~uuid ~enabled = D.info "skeleton Observer.set_enabled" ; ()

  let set_attributes ctx ~dbg ~uuid ~attributes = ()

  let set_endpoints ctx ~dbg ~uuid ~endpoints = D.info "skeleton Observer.set_endpoints" ; ()

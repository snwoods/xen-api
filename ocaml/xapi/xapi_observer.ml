...
module Xapi_cluster = struct
  ...
  module Observer = struct
    let create ~__context ~uuid ~name_label ~attributes ~endpoints ~enabled =
      debug "Cluster Observer.create %s" uuid ;
    ...
    let destroy ~__context ~uuid =
      debug "Cluster Observer.destroy %s" uuid ;
      let module S = (val local_client ~__context : XAPI_CLUSTER) in
      let dbg = Context.string_of_task __context in
      S.Observer.destroy dbg uuid

    let set_enabled ~__context ~uuid ~enabled =
      debug "Cluster Observer.set_enabled %s" uuid ;
      let module S = (val local_client ~__context : XAPI_CLUSTER) in
      let dbg = Context.string_of_task __context in
      S.Observer.set_enabled dbg uuid enabled

    let set_attributes ~__context ~uuid ~attributes =
      debug "Cluster Observer.set_attributes %s" uuid ;
      let module S = (val local_client ~__context : XAPI_CLUSTER) in
      let dbg = Context.string_of_task __context in
      S.Observer.set_attributes dbg uuid attributes

    let set_endpoints ~__context ~uuid ~endpoints =
      debug "Cluster Observer.set_endpoints %s" uuid ;
      let module S = (val local_client ~__context : XAPI_CLUSTER) in
      let dbg = Context.string_of_task __context in
      S.Observer.set_endpoints dbg uuid endpoints

    let init ~__context =
      debug "Cluster Observer.init" ;
      let module S = (val local_client ~__context : XAPI_CLUSTER) in
      let dbg = Context.string_of_task __context in
      S.Observer.init dbg

    let set_trace_log_dir ~__context ~dir =
      debug "Cluster Observer.set_trace_log_dir" ;
      let module S = (val local_client ~__context : XAPI_CLUSTER) in
      let dbg = Context.string_of_task __context in
      S.Observer.set_trace_log_dir dbg dir

    let set_export_interval ~__context ~interval =
      debug "Cluster Observer.set_export_interval" ;
      let module S = (val local_client ~__context : XAPI_CLUSTER) in
      let dbg = Context.string_of_task __context in
      S.Observer.set_export_interval dbg interval

    let set_max_spans ~__context ~spans =
      debug "Cluster Observer.set_max_spans" ;
      let module S = (val local_client ~__context : XAPI_CLUSTER) in
      let dbg = Context.string_of_task __context in
      S.Observer.set_max_spans dbg spans

    let set_max_traces ~__context ~traces =
      debug "Cluster Observer.set_max_traces" ;
      let module S = (val local_client ~__context : XAPI_CLUSTER) in
      let dbg = Context.string_of_task __context in
      S.Observer.set_max_traces dbg traces

    let set_max_file_size ~__context ~file_size =
      debug "Cluster Observer.set_max_file_size" ;
      let module S = (val local_client ~__context : XAPI_CLUSTER) in
      let dbg = Context.string_of_task __context in
      S.Observer.set_max_file_size dbg file_size

    let set_host_id ~__context ~host_id =
      debug "Cluster Observer.set_host_id" ;
      let module S = (val local_client ~__context : XAPI_CLUSTER) in
      let dbg = Context.string_of_task __context in
      S.Observer.set_host_id dbg host_id

    let set_compress_tracing_files ~__context ~enabled =
      debug "Cluster Observer.set_compress_tracing_files" ;
      let module S = (val local_client ~__context : XAPI_CLUSTER) in
      let dbg = Context.string_of_task __context in
...
module Xapi_storage_script = struct
  ...
  module Observer = struct
    let create ~__context ~uuid ~name_label ~attributes ~endpoints ~enabled =
      debug "Storage_script Observer.create %s" uuid ;
      let dbg = Context.string_of_task __context in
      Client.Observer.create dbg uuid name_label attributes endpoints enabled

    let destroy ~__context ~uuid =
      debug "Storage_script Observer.destroy %s" uuid ;
      let dbg = Context.string_of_task __context in
      Client.Observer.destroy dbg uuid

    let set_enabled ~__context ~uuid ~enabled =
      debug "Storage_script Observer.set_enabled %s" uuid ;
      let dbg = Context.string_of_task __context in
      Client.Observer.set_enabled dbg uuid enabled

    let set_attributes ~__context ~uuid ~attributes =
      debug "Storage_script Observer.set_attributes %s" uuid ;
      let dbg = Context.string_of_task __context in
      Client.Observer.set_attributes dbg uuid attributes

    let set_endpoints ~__context ~uuid ~endpoints =
      debug "Storage_script Observer.set_endpoints %s" uuid ;
      let dbg = Context.string_of_task __context in
      Client.Observer.set_endpoints dbg uuid endpoints

    let init ~__context =
      debug "Storage_script Observer.init" ;
      let dbg = Context.string_of_task __context in
      Client.Observer.init dbg

    let set_trace_log_dir ~__context ~dir =
      debug "Storage_script Observer.set_trace_log_dir" ;
      let dbg = Context.string_of_task __context in
      Client.Observer.set_trace_log_dir dbg dir

    let set_export_interval ~__context ~interval =
      debug "Storage_script Observer.set_export_interval" ;
      let dbg = Context.string_of_task __context in
      Client.Observer.set_export_interval dbg interval

    let set_max_spans ~__context ~spans =
      debug "Storage_script Observer.set_max_spans" ;
      let dbg = Context.string_of_task __context in
      Client.Observer.set_max_spans dbg spans

    let set_max_traces ~__context ~traces =
      debug "Storage_script Observer.set_max_traces" ;
      let dbg = Context.string_of_task __context in
      Client.Observer.set_max_traces dbg traces

    let set_max_file_size ~__context ~file_size =
      debug "Storage_script Observer.set_max_file_size" ;
      let dbg = Context.string_of_task __context in
      Client.Observer.set_max_file_size dbg file_size

    let set_host_id ~__context ~host_id =
      debug "Storage_script Observer.set_host_id" ;
      let dbg = Context.string_of_task __context in
      Client.Observer.set_host_id dbg host_id

    let set_compress_tracing_files ~__context ~enabled =
      debug "Storage_script Observer.set_compress_tracing_files" ;
      let dbg = Context.string_of_task __context in

...
module TraceContext = struct
  ...
  let with_added_baggage k v ctx =
    let new_baggage = baggage_of ctx
...
module Span = struct
  ...
  let start ?(attributes = Attributes.empty)
    ...
    let extra_context_with_depth = TraceContext.(with_added_baggage depth_key (string_of_int depth) extra_context) in
    ...
    let context =
      ...
      | Some tc ->
        let tc_with_depth = TraceContext.(with_added_baggage depth_key (string_of_int depth) tc) in
...
module Tracer = struct
  ...
  let span_of_span_context context name : Span.t =
    ...
    let new_tc = TraceContext.(with_added_baggage depth_key (string_of_int new_depth) tc) in
    ...
    if (TracerProvider.get_current ()).enabled then
      ...
      | Some parent ->
          ...
          |> Option.map (fun existing_span ->
                 ...
                 let new_depth = TraceContext.baggage_depth_of parent_trace_context + 1 in
                 let new_context : SpanContext.t =
                    let trace_context = TraceContext.(with_added_baggage depth_key (string_of_int new_depth) span.Span.context.trace_context) in
                   SpanContext.context
    ...
    let with_tracing ?attributes ~name carrier f =
      ...
      let continue_with_child = function
        | Some child ->
            ...
            in
            let new_depth = TraceContext.baggage_depth_of trace_context' + 1 |> string_of_int in
            let trace_context'' =
              TraceContext.(with_added_baggage depth_key new_depth trace_context') in

...
let observer_config_dir =
  ...
  let dir, component =
    Constants.(observer_config_dir, observer_component_xapi_storage_script)
  in
...
module Observer = struct
  let create _ _ _ _ _ _ enabled =
    ...
    let* () =
      info (fun m ->
          m "xapi_storage_script actual observer.create use_observer=%b"
            config.use_observer
      )
    in
    return ()
  ...
  let destroy _ _ _ =
    ...
    let* () =
      info (fun m ->
          m "xapi_storage_script actual observer.destroy use_observer=%b"
            config.use_observer
      )
    in
    return ()
  ...
  let set_enabled _ _ _ enabled =
    ...
    let* () =
      info (fun m ->
          m "xapi_storage_script actual observer.set_enabled use_observer=%b"
            config.use_observer
      )
    in
    return ()
  ...
  let set_endpoints _ _ _ _ =
    wrap
    @@
    let* () =
      info (fun m ->
          m
            "xapi_storage_script actual observer.set_endpoints doing nothing \
             use_observer=%b"
            config.use_observer
      )
    in

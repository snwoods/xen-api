...
let observer_is_component_enabled () =
  let* () = info (fun m -> m "Entered observer_is_component_enabled") in
  let is_enabled () =
...
let watch_volume_plugins ~volume_root ~switch_path ~pipe () =
  let* () = info (fun m -> m "watch_volume_plugins") in
  let create volume_plugin_name =
    if Base.Hashtbl.mem servers volume_plugin_name then
      ...
      let* () = info (fun m -> m "watch_volume_plugins bind") in
...
let self_test_plugin ~root_dir plugin =
  let* () = info (fun m -> m "self_test_plugin bind") in
...
let main ~root_dir ~state_path ~switch_path =
  let* () = info (fun m -> m "xapi_storage_script main") in
  ...
  in
  let* () = info (fun m -> m "xapi_storage_script bind") in
  [
    ( "volume plugins"
...
module D = Debug.Make (struct let name = "xapi-storage-script" end)
...
module XapiStorageScript : Observer_helpers.Server_impl = struct
  ...
  let create _context ~dbg:_ ~uuid:_ ~name_label:_ ~attributes:_ ~endpoints:_ ~enabled =
    ...
    D.info "xapi_storage_script actual observer.create use_observer=%b" config.use_observer

  let destroy _context ~dbg:_ ~uuid:_ =
    config.use_observer <- false ;
    D.info "xapi_storage_script actual observer.destroy use_observer=%b" config.use_observer

  let set_enabled _context ~dbg:_ ~uuid:_ ~enabled =
    config.use_observer <- enabled ;
    D.info "xapi_storage_script actual observer.set_enabled use_obsegrver=%b" config.use_observer

  let set_attributes _context ~dbg:_ ~uuid:_ ~attributes:_ = ()

  let set_endpoints _context ~dbg:_ ~uuid:_ ~endpoints:_ =
    D.info "xapi_storage_script actual observer.set_endpoints use_observer=%b" config.use_observer
end
...
let () =
  ...
  in
  D.info "Started service on %s" queue_name ;
  let (_ : Thread.t) =
    Thread.create (fun () -> Xcp_service.serve_forever s) ()
  in

  let main =
    let* () = info (fun m -> m "Starting xapi_storage_script") in

...
if configs:
  ...
  def tracer_of_config(path):
    ...
    # Temporary: service.name is required and xapi is failing to write default attributes to the observer.conf
    if "service.name" not in config_otel_resource_attributes:
        config_otel_resource_attributes = "service.name=sm"
  ...
  def span_of_tracers(wrapped=None, span_name_prefix=""):
    ...
    def wrapper(wrapped, instance, args, kwargs):
      ...
      if not tracers:
        ...
        # TODO Broadcast the span across all tracers
        return result
